...
- **Track:** relaxation only via explicit client message logged verbatim as an event; every generated piece records which spec version it was checked against. Known holes this law does not yet cover: the rendered image (FLANK-01) and the days to go SILENT (FLANK-02).
...
- **Why:** Truth Law's fence + Idea Law's nouns. `founder_taste.json` already presumes this file ("hashtags ONLY from the truth pack's real list") — as a client-profile organ it doesn't exist yet.
...
- **Archive:** NEW schema `12_data_shapes/truth_pack_v1.schema.json` (verified absent today — TO BUILD). Honest census: it is NOT the only schema this pyramid needs — red_lines, goals, moments_bank, audience_mirror, state, taste and the gap_report have no schema either, so the `validate.py` gate is unenforceable for 7/10 organs until all eight land (GAP-11). Every entry cites post shortcode, website, or event ULID.
...
- **Track:** monthly comment re-pull; detected complaint pings the client as a service. Known blind spot: this organ hears Instagram only (FLANK-05).
...
**5. PROFILE COMPOSER** — `scripts/build_brand_profile.py` (TO BUILD): one deterministic script composing all organs from raw + `derived/voice_stats` + sector defaults + the pick + the answers. The SAME script backfills albaik with everything `confidence:inferred`.
- **Why:** the pyramid is only real when one script materializes it for ANY brand — built top-down, runs bottom-up.
- **Source:** inferred.
- **Diagnose:** output validates via `scripts/validate.py` (exit 0 or not done — a real gate only once the 8 missing organ schemas land, GAP-11); the completeness report IS the next gap_report — the albaik hole becomes a field-count, not a vibe.
...
- **Why:** the YES must cost seconds, not meetings. An undone gate is an unearned YES. Validated on the founder (sessions complete, avg 2.95 baseline) — does not exist for clients yet: the single highest-leverage missing piece.
...
- **Track:** it IS the tracking substrate — taste, trust meter, state, churn, priors are all derived views. Known hole: no commercial event types yet (FLANK-03).
...
**@ Layer 2 — truth gates**
- **INFERENCE QUARANTINE:** `scripts/fingerprint_status.py` (to build, modeled on verify_ship_ready) renders voice GREEN / identity RED side-by-side — so 164 observations can never again impersonate completeness. Identity fields (who-speaks, USP, goals, red lines) may not be inferred at all.
...
- **DEADLY-DEFAULTS TABLE — TO BUILD:** target path `15_cultural_specs/defaults/brand_override_defaults_v1.yaml` — verified ABSENT on disk 2026-06-11; nothing may cite it until it exists. Spec for the build: written deadly-vs-survivable pairs: face_visibility → `never`; mixed_gender → family-only; family-voice («أمي جابت...») OFF (parked, Mohamed's call); masculinity framing OFF (parked); offers on emotional occasions OFF; prices OFF until truth-confirmed; comment-quoting OFF. Any deadly field running non-conservative without a client event = release block.

**@ Layer 4 — rot and memory gates**
- **TTL REGISTRY:** offers/prices 7d · products 30d · channels 60d · bio 90d · voice stats 180d · red lines NEVER auto-expire but re-confirm every 5th touch («لسه ما ننشر وجوه العائلة — صح؟», one tap). Past-TTL auto-downgrades confirmed→inferred → uncitable by the citation gate. Gates compose; no human has to remember. Known hole: TTLs are flat — no season-scoped truth (a "confirmed" Shawwal price stays citable into Ramadan), FLANK-02. `scripts/staleness_report.py`: expired-but-cited count must be 0.
- **DRIFT-WATCH ALARM:** zero diffs 6 months on an active client = our monitoring broke, not their brand froze.
- **CRYSTALLIZE LOOP:** weekly pass over all `events/` — any coded reason ×3 across clients drafts a kill/default change, queued to Mohamed as a 60-second yes/no. Tripwire: `founder_taste.json _meta` >2 weeks behind the last session = the loop itself is broken. (Standing evidence: RABIE's 2/5 ruling survives only as an inline note in voice_birth_week.json — events first, notes never.)
...
4. **Voice path = BIRTH:** 3 personas proposed in `voice_birth_week.json` (A بنت نجد الدافئة / B الحرفية الأنيقة / C خفيفة الدم); C regenerated after RABIE's 2/5 truth-error rating — correction already logged in-file, to be re-logged as an event.
...
- **GAP-02 — truth pack: 0/0/0 (products/prices/channels).** And `truth_pack_v1.schema.json` does not exist in `12_data_shapes/` (verified — 16 schemas present, this one absent). Build the schema, mine 110 beverage obs into `inferred` candidates.
...
- **GAP-06 — verdict loop unbuilt:** 0 outcome events for albaik, 0 for any client; the 60-second client gate does not exist — highest-leverage missing machine.
...
- **GAP-08 — composer missing:** no `scripts/build_brand_profile.py`, no `scripts/fingerprint_status.py` (`client_intake.py`, `validate.py`, `verify_ship_ready.py`, `build_truth_pack.py` exist to build on).
- **GAP-09 — audience mirror uncounted:** albaik comment corpus size unknown — run the comments surface or mark thin.
- **GAP-10 — consent scope:** albaik is a benchmark, not a client. Dry-run is INTERNAL ONLY; nothing client-facing ships from it.
- **GAP-11 — schema coverage 2/10 organs.** Only fingerprint (brand_fingerprint_v1) and cultural_overrides (cultural_spec_v1) have schemas on disk. truth_pack_v1 absent (GAP-02); red_lines, goals, moments_bank, audience_mirror, state, taste and the gap_report have none — the `validate.py` "exit 0 or not done" gate is unenforceable for 7/10 organs until all eight missing schemas land. TO BUILD with the composer; until then no organ without a schema may be called "validated."
...
*Design-level holes filed as standing gap cards. None blocks the eyes-test; each blocks "production-ready." Two are trust-killers (FLANK-01, FLANK-02); none may be closed by AI judgment.*

- **FLANK-01 — THE VISUAL HALF IS UNGATED (trust-killer).** Every Truth/Safety gate in this document scans TEXT; face_visibility, modesty, mixed_gender, and family-visibility violations actually live in the rendered IMAGE (the 88 chains), with no deterministic check between render and human eyes. Worse, an undecided Truth Law question: an AI-generated photo of "her jurisha" is not her jurisha — a customer who orders off a beautiful synthetic image and receives the real product is the Truth Law violated in pixels. `open_gap image_pairing` admits captions are half a post; the immune system currently guards only that half. Until a visual gate exists: human eyes ARE the visual gate, named explicitly; no batch (L1) may contain AI-rendered product imagery; the real-vs-synthetic-product ruling parks in Mohamed's queue (parked-rulings ledger).
- **FLANK-02 — NO NEGATIVE-SPACE CALENDAR (trust-killer).** Every calendar node says when to POST; nothing says when to GO SILENT. A scheduled celebratory post on a national mourning day passes every gate in this document — the exact screenshotable public embarrassment the Safety Law exists to prevent. Needed: a mourning/crisis blackout switch (one flag halts ALL publishing including approved batches), prayer-time and late-night send etiquette, Ramadan's inverted hours. Related rot: truth has flat TTLs only — no season-scoped truth (Ramadan menu/hours that flip and flip back), so the TTL registry happily keeps a confirmed Shawwal price live into Ramadan. TO BUILD before any batch consent goes live.
- **FLANK-03 — THE COMMERCIAL SPINE IS ABSENT.** The pyramid models the client's YES but never what he BUYS: no package/tier, no posts-per-week commitment, no price, no contract or renewal date, no `payment_received` / `renewal` / `scope_change` event types in the ledger. Layer 4 calls the monthly report "the renewal artifact" and tracks cost-per-YES with no revenue side — a repeated YES from a client who stopped paying is churn the instruments literally cannot see. A Saudi SME relationship dies over a disputed invoice or a late payment more often than over a bad caption.
- **FLANK-04 — NO COMPETITOR WATCH.** Zero nodes. The owner watches 2-3 named rivals (the kabsa kitchen two streets over, the rival on HungerStation's front page); half his real requests arrive as «سويلي زي ذا» + a forwarded competitor post. No `competitor_set` per client, no intake path for a forwarded-competitor request, and the `very_normal` kill ("if any brand could post it, it's dead") has no competitor corpus to test against — the 100 benchmark accounts are sector-generic, not HIS street.
- **FLANK-05 — HIS CUSTOMERS OFF-INSTAGRAM ARE INVISIBLE.** audience_mirror reads IG comments (jurisha: 10) + one WhatsApp question — yet the doc itself routes orders to جاهز/هنقرستيشن and never reads the reviews/ratings THERE, or Google Maps. For a delivery-first F&B SME the delivery-app review section IS the customer voice and the loudest reputation surface; a 3.2-star «وصلت باردة» reality makes every warm-family-kitchen caption a lie the system cannot detect. This is the missing feed-vs-door check — the gap between the image and what actually arrives.
