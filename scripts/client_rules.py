...
_ROLE = r"(?:أخت[يكه]?|أخ[يوكه]?|الأخت|الأخ|جد(?:ت[يكه]|[يهك])?|الجد|الجدة|أم[يكه]?|الأم|أب[يوكه]?|الأب|الوالد[ةه]?|" \
...
_ROLE_B = _ROLE + r"(?![\u0621-\u064a])"  # Arabic-letter boundary: جد must not match inside جدة (Jeddah) — golden over-block 1, 2026-07-07, court-applied
FAMILY_VOICE = re.compile(_ROLE_B + r"[^.!؟\n]{0,30}?" + _SAY + r"\s*[:،]?|" + _SAY + r"[^.!؟\n]{0,15}?" + _ROLE_B)
