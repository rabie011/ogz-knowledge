...
# ── CULTURAL RED LINES (Mohamed's rules, 2026-06-07 human review) ──────────────
# HARD BLOCK — these are never acceptable in any Saudi brand caption.
CULTURAL_RED_LINES = {
    'clothing_removal': ['تفك الخمار', 'تفك فيها الخمار', 'خلع', 'تخلع', 'نزع', 'ازالة ملابس',
                         'إزالة الملابس', 'تنزع', 'الحجاب', 'يتعرى', 'عاري', 'تكشف'],
    'bedroom_intimacy': ['السرير', 'سرير', 'غرفة النوم', 'الفراش', 'بالفراش'],
    'exploiting_weakness': ['ضعفك', 'ضعفهم', 'نقاط ضعفك', 'استغلال ضعف', 'هشاشتك'],
}

# Max caption length by sector (Mohamed: captions are too long — #1 complaint)
SECTOR_MAX_CHARS = {
    'f_and_b': 160, 'fashion': 400, 'beauty_personal_care': 300,
    'retail_lifestyle': 220, 'real_estate': 280, 'healthcare_wellness': 200,
}
...
def check(text: str, brand: str = None, occasion: str = 'evergreen') -> dict:
    ...
    # ── Check 7b: RED LINES (Mohamed's rules — instant hard block) ────────
    red_line_hit = []
    for category, terms in CULTURAL_RED_LINES.items():
        for t in terms:
            if t in text:
                red_line_hit.append(f'{category}:{t}')
    if red_line_hit:
        hard_blocked = True
        checks.append({'check': 'red_line', 'passed': False, 'weight': 30,
                        'detail': f'RED LINE: {red_line_hit}'})
    else:
        checks.append({'check': 'red_line', 'passed': True, 'weight': 30, 'detail': 'Clean'})

    # ── Check 7c: Length cap (Mohamed: captions too long — #1 complaint) ──
    max_chars = SECTOR_MAX_CHARS.get(sector, 250)
    if len(text) > max_chars:
        checks.append({'check': 'length', 'passed': False, 'weight': 10,
                        'detail': f'Too long: {len(text)} chars (max {max_chars})'})
        fixes.append('too_long')
    else:
        checks.append({'check': 'length', 'passed': True, 'weight': 10,
                        'detail': f'{len(text)} chars (OK)'})
